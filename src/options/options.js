...
function applyEnglishDashUI() {
  const ops = [
    ...
    ['#downloadFolder', 'placeholder', 'E.g.: bia-qa (subfolder inside Downloads)'],
  ];
...
<hr><p style="color:#6b7280;font-size:11px">${tt('Relatório gerado pela Bia', 'Report generated by Bia')}</p>
  ...
  saveDashDownload(new Blob([html], { type: 'text/html' }), `bia-qa-relatorio-executivo-${new Date().toISOString().slice(0, 10)}.html`);
}
...
<section>
  ...
  <p><b>Where files are saved:</b> in Settings → Agent you can set a subfolder inside Downloads (e.g. <code>bia-qa</code>) for videos and screenshots, or check "Ask where to save" to pick any folder via Chrome's picker on each download. For security, Chrome can't write outside Downloads without that picker.</p>
...
function exportLibrary() {
  chrome.storage.local.get(['savedTests'], (r) => {
    saveDashDownload(
      ...
      `bia-qa-testes-${new Date().toISOString().slice(0, 10)}.json`
    );
  });
}
